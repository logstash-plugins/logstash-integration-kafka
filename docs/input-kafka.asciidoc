...
Due to the limitation of https://javadoc.io/doc/org.apache.kafka/kafka-clients/2.5.1/org/apache/kafka/clients/consumer/ConsumerConfig.html[Kafka client],
this plugin does not support using a proxy when communicating to Kafka broker.
This plugin only supports using a proxy when communicating to the Schema Registry, see <<plugins-{type}s-{plugin}-schema_registry_proxy>>
