...
By not setting this value, the plugin uses the default behavior of the Kafka client, which is `uniform_sticky`.
If a record explicitly specifies a partition, that partition is always used.
Otherwise, the producer selects a sticky partition, which remains the same for the duration of a batch and changes once the batch is full or flushed.

Available options for choosing a partitioning strategy are as follows:

* `default` is an alias for `uniform_sticky`
