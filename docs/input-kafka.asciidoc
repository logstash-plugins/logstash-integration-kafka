...
[NOTE]
====
The following options are not applicable in `share_group` mode and will be ignored or
raise a configuration error if set to non-default values:
...
Schema Registry integration (`schema_registry_url`) is also not supported.
====
