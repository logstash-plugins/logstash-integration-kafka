...
* `uniform_sticky` hashes the `message_key` of an event to get the partition. When no message key is present, the plugin switches to a new partition after every batch.size bytes are produced, ensuring uniform distribution across partitions. This is the default strategy of Kafka client.
