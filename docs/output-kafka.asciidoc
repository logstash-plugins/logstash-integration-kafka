...
A {logstash-ref}/field-references-deepdive.html[field reference] to a hash whose entries are added to
the message as Kafka headers, the values being used verbatim (no `%{}` interpolation). This allows
forwarding headers that are not known in advance, such as the headers of a consumed record captured
by the Kafka input's
{logstash-ref}/plugins-inputs-kafka.html#plugins-inputs-kafka-decorate_events[`decorate_events => extended`] mode:
[source,ruby]
----------------------------------
    message_headers_field => "[@metadata][kafka][headers]"
----------------------------------

These headers are appended after the ones from `message_headers` (Kafka allows duplicate names).
If the referenced field is absent, the event is sent without additional headers. Entries with an
empty key or `nil` value are skipped; non-scalar values, or a field that does not contain a hash,
